...
class ImageSlider extends Component<PropsType, StateType> {
  ...
  _renderImage = (image: any, index: number) => {
    ...
    const { onPress, customSlide, resizeMode } = this.props;
    ...
    const imageObject = typeof image === 'string' ? { uri: image, headers:{ Authorization: 'someAuthToken' }, priority: FastImage.priority.normal, } : image;

    const imageComponent = (
      <FastImage key={index} resizeMode={resizeMode} source={imageObject} style={[imageStyle]} />
    );
